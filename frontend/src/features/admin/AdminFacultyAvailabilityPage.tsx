...
export function AdminFacultyAvailabilityPage() {
  ...
  function openEditScheduleForm(schedule: AvailabilitySchedule) {
  ...
  function openEditExceptionForm(exception: AvailabilityException) {
  ...
  async function handleScheduleSubmit(
    ...
    if (scheduleFormValues.startTime >= scheduleFormValues.endTime) {
      setScheduleFormError("End time must be later than start time.")
      return
    }
  ...
  async function handleExceptionSubmit(
    ...
    if (
      ...
      setExceptionFormError("End time must be later than start time.")
      return
    }
  ...
  async function handleDeleteSchedule(schedule: AvailabilitySchedule) {
    ...
    try {
      ...
      window.alert("Unable to delete this schedule. Please try again.")
    }
  }
  ...
  async function handleDeleteException(exception: AvailabilityException) {
    const description = exception.startTime
      ...
      : `${formatExceptionDate(exception.date)} (full day)`

    const confirmed = window.confirm(
      `Delete this availability exception for ${description}?`,
    )
    ...
    try {
      ...
      window.alert("Unable to delete this exception. Please try again.")
    }
  }
  ...
  if (facultyQuery.isError || departmentsQuery.isError) {
    return (
      <ErrorState message="Unable to load faculty availability." />
    )
  }
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-10 p-6">
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-muted-foreground">
            Configure this faculty member's weekly availability and
      ...
      <section className="space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Active schedules generate appointment slots for students.
            </p>
          </div>
        ...
        {schedulesQuery.isError && (
          <ErrorState message="Unable to load this faculty member's schedules." />
        )}
          ...
          schedules.length === 0 && (
            <section className="rounded-xl border border-dashed p-8 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                Add a weekly schedule so students can see this faculty
                member's available appointment slots.
              </p>
              <Button className="mt-5" onClick={openCreateScheduleForm}>
                Add first schedule
              </Button>
            </section>
          )}
          ...
          schedules.length > 0 && (
            <div className="space-y-3">
              {schedules.map((schedule) => (
                <article key={schedule.id} className="rounded-xl border p-5">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {schedule.slotDuration}-minute appointments
                      </p>
                    </div>
        ...
        {isScheduleFormOpen && (
          <section className="rounded-xl border p-6">
            <div className="mb-6 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Set when students can request appointments with this faculty
                member.
              </p>
            </div>
            ...
            <form onSubmit={handleScheduleSubmit} className="space-y-6">
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="adminSlotDuration">Slot duration</Label>
                  <select
                    ...
                    onChange={(event) =>
                      setScheduleFormValues((current) => ({
                        ...current,
                        slotDuration: Number(event.target.value) as
                          | 15
                          | 30
                          | 45
                          | 60,
              ...
              <label className="flex items-center gap-3 text-sm">
                ...
                />
                <span>
                  <span className="font-medium">Schedule is active</span>
      ...
      <section className="space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Block a full day or a specific time range from the normal
              schedule.
            </p>
          </div>
        ...
        {exceptionsQuery.isError && (
          <ErrorState message="Unable to load this faculty member's availability exceptions." />
        )}
        ...
        {!exceptionsQuery.isLoading &&
          ...
          exceptions.length === 0 && (
            <section className="rounded-xl border border-dashed p-8 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                Add an exception when this faculty member needs to block a
                date or time from the normal schedule.
              </p>
              <Button
                className="mt-5"
                variant="outline"
          ...
          exceptions.length > 0 && (
            <div className="space-y-3">
              {exceptions.map((exception) => (
                <article key={exception.id} className="rounded-xl border p-5">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-2">
                      <h3 className="font-semibold">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => openEditExceptionForm(exception)}
                      >
                        <Pencil className="mr-2 size-4" />
                        Edit
                      </Button>
        ...
        {isExceptionFormOpen && (
          <section className="rounded-xl border p-6">
            <div className="mb-6 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Temporarily block availability for this faculty member.
              </p>
            </div>

            <form onSubmit={handleExceptionSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="adminExceptionDate">Date</Label>
                <Input
              ...
              <label className="flex items-center gap-3 text-sm">
                ...
                />
                <span>
                  <span className="font-medium">Full-day exception</span>
                  <span className="block text-muted-foreground">
                    Block the entire availability for this date.
                  </span>
                </span>
              </label>
              ...
              {!exceptionFormValues.isFullDay && (
                <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="adminExceptionEndTime">End time</Label>
                    <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="adminExceptionReason">Reason</Label>
                <Input
              ...
              {exceptionFormError && (
                <p className="text-sm text-destructive">{exceptionFormError}</p>
              )}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit" disabled={isExceptionSaving}>
                  {isExceptionSaving
                    ? "Saving..."
                    : editingException
                      ? "Save changes"
